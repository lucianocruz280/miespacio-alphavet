...
export default function MyServiceDetail({ serviceId, category }: MyServiceDetailProps) {
  ...
  const service = (data as any)?.data
  ...
  const title = isGrooming ? "Servicio de Estética" : "Consulta Médica"
      ...
      <Card className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-500">Mascota: {service.pet?.name}</p>
              </div>
            </div>
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              <div className="flex items-center gap-3 text-slate-700">
                ...
                <span>{service.branch?.name || "Sucursal Principal"}</span>
              </div>
              <div className="flex items-center gap-3 text-slate-700">
              ...
              <div className="flex items-center gap-3 text-slate-700">
                ...
                <span>Atendido por: {isGrooming ? service.groomer?.name : service.veterinarian?.name}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 h-full">
            <h3 className="font-semibold text-slate-900 mb-2">
              {isGrooming ? "Notas del Servicio" : "Motivo de la Consulta"}
            </h3>
            <p className="text-slate-700 italic border-l-4 border-slate-300 pl-3 py-1">
              "{isGrooming ? (service.notes || "Sin notas adicionales.") : service.reason}"
            </p>

            {!isGrooming && service.diagnosis && (
              <div className="mt-6">
                <h3 className="font-semibold text-slate-900 mb-2">Diagnóstico</h3>
        ...
        {!isGrooming && service.prescriptions && service.prescriptions.length > 0 && (
          <div className="mb-8 border-t border-slate-100 pt-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {service.prescriptions.map((prescription: any) => (
                <div key={prescription.id} className="p-4 bg-white border border-slate-200 rounded-xl hover:border-indigo-300 transition-colors">
                  ...
                  <div className="mt-2 text-sm text-slate-600 space-y-1">
                    <p><span className="font-medium">Dosis:</span> {prescription.dose} {prescription.unit}</p>
                    <p><span className="font-medium">Frecuencia:</span> Cada {prescription.frequencyHours} horas</p>
                    {prescription.durationDays && <p><span className="font-medium">Duración:</span> {prescription.durationDays} días</p>}
        ...
        {(service.items?.length > 0) && (
          <div className="border-t border-slate-100 pt-6">
            <h3 className="font-semibold text-slate-900 mb-4">Servicios/Productos Aplicados</h3>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700">
                  {service.items.map((item: any) => (
                    <tr key={item.id}>
                      <td className="py-3 px-4">{item.service?.name || item.product?.name || "Servicio"}</td>
