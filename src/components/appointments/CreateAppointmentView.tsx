...
const CreateAppointmentView = () => {
   ...
   if (!code) return <div>{t('appointments.loading')}</div>
    return (
        <main className="max-w-6xl mx-auto p-4 md:p-8 space-y-8">

            <div className="hidden md:flex items-center gap-3">
                <span className="text-sm font-medium text-slate-900">
                    {t('appointments.stepProgress', { current: currentStepNumber, total: totalSteps })}
                </span>
                ...
                <span className="text-sm text-slate-500">
                    {t(`appointments.header.stepper.${stepper.current.id === 'base' ? 'baseInfo' : stepper.current.id}`)}
                </span>
            </div>
